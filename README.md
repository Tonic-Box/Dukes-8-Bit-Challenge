...
- **No per-frame allocation:** colors are hoisted into constants and entities are reused in fixed arrays, keeping peak memory low (another tiebreaker).
- **JDK-only dependencies:** rendering uses AWT, which ships with the runtime and is not counted as an external library.
