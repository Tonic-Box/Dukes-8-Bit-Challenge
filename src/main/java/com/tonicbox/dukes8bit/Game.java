 ...
 * player, and a pool of enemies as flat primitive data. The player glides on a
...
final class Game {
    ...
    private void startNewGame() {
        ...
        pauseSelection = 0;
        quaffHeld = buyHeld = enterHeld = escHeld = false;
        quaffRequested = buyRequested = talkRequested = false;
        regenTimer = 0f;
     ...
     * flags while playing, and the menu choices in the shop, pause, and death states.
     */
     ...
     * step when a direction is held and the previous slide has finished.
     */
    ...
    /** Moves Duke if the target tile is open; enemies and walls block, stairs descend. */
    ...
    private void performAttack() {
        ...
        for (int i = enemyCount - 1; i >= 0; i--) {
            ...
            if (Math.max(dx, dy) == 1) {
                enemyHp[i] -= Math.max(1, playerAtk + random.nextInt(3));
                enemyHit[i] = 1f;
                if (enemyHp[i] <= 0) {
                    gold += goldReward(enemyType[i]);
                    grantXp(xpReward(enemyType[i]));
                    removeEnemy(i);
                }
            }
        }
     ...
     * see Duke, then steps toward him unless it is already adjacent.
     */
    private void enemyStep() {
        for (int i = 0; i < enemyCount; i++) {
            enemyPrevX[i] = enemyX[i];
            enemyPrevY[i] = enemyY[i];
            int distance = Math.abs(playerX - enemyX[i]) + Math.abs(playerY - enemyY[i]);
     ...
     * independent of how often it moves.
     */
    private void enemyContacts(long deltaMillis) {
        for (int i = 0; i < enemyCount; i++) {
            ...
            if (Math.abs(playerX - enemyX[i]) + Math.abs(playerY - enemyY[i]) == 1 && enemyCooldown[i] <= 0f) {
                playerHp -= Math.max(1, enemyAttack(enemyType[i]) + random.nextInt(2));
    ...
    private int rollType() {
        int roll = genRandom.nextInt(floor + 2);
        return roll < 2 ? BUG : roll < 4 ? NULLPTR : LEAK;
    }
    ...
    private void placeMerchant(int floorWidth, int floorHeight) {
        ...
        for (int attempt = 0; attempt < 200; attempt++) {
            ...
            if (Math.abs(x - playerX) + Math.abs(y - playerY) < 4) {
                continue;
            }
            merchantX = x;
            merchantY = y;
    ...
    boolean adjacentToMerchant() {
        return merchantX >= 0 && Math.abs(playerX - merchantX) + Math.abs(playerY - merchantY) == 1;
    }
    ...
    private void spawnEnemies(int floorWidth, int floorHeight) {
        ...
        for (int attempt = 0; attempt < target * 20 && enemyCount < target; attempt++) {
            ...
            if (Math.abs(x - playerX) + Math.abs(y - playerY) < 6) {
                continue;
            }
            enemyX[enemyCount] = x;
            enemyY[enemyCount] = y;
            enemyPrevX[enemyCount] = x;
            enemyPrevY[enemyCount] = y;
            enemyType[enemyCount] = rollType();
            enemyHp[enemyCount] = enemyMaxHp(enemyType[enemyCount]);
            enemyHit[enemyCount] = 0f;
            aggroed[enemyCount] = false;
            enemyCooldown[enemyCount] = 0f;
            enemyCount++;
        }
    }
     ...
     * line to every tile within the view radius, stopping at the first wall.
     * Visible tiles are also marked permanently explored.
     */
    private void computeFieldOfView() {
        for (int i = 0; i < visible.length; i++) {
            visible[i] = false;
        }
        for (int y = playerY - FOV_RADIUS; y <= playerY + FOV_RADIUS; y++) {
            for (int x = playerX - FOV_RADIUS; x <= playerX + FOV_RADIUS; x++) {
                int dx = x - playerX;
                int dy = y - playerY;
                if (inBounds(x, y) && dx * dx + dy * dy <= FOV_RADIUS * FOV_RADIUS) {
                    castLight(x, y);
                }
            }
        }
